...
<link type="text/css" rel="stylesheet"
	href="css/patrolTaskMgr/alterPatrolTask.css" />
<jsp:include page="../../comm/comm_easyUI_js.jsp" />
</head>

<body>
<form method="post" action="updateTask_PatrolTask">
	<input id="circuit_id" value="<s:property value="pollTask.circuit.circuit_id"/>" type="hidden"/>
	<table class="tab">
		<tr>
			<td class="tdodd">任务名称：</td>
			<td><input type="text" name="pollTask.task_name" value="<s:property value="pollTask.task_name"/>" /></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td><textarea id="ta1" class="ta1"><s:property value="message"/></textarea><input
				type="image" id="serach" src="file/icons/search.png" /></td>
		</tr>
		<tr>
			<td class="tdodd">起始杆号：</td>
			<td><input type="text" value="<s:property value="pollTask.circuit.start_tower_id"/>" /></td>
		</tr>
		<tr>
			<td class="tdodd">终止杆号：</td>
			<td><input type="text" value="<s:property value="pollTask.circuit.end_tower_id"/>" /></td>
		</tr>
		<tr>
			<td class="tdodd">下 发 人：</td>
			<td class="label"><label><s:property value="pollTask.fromSysUser.user_name"/></label></td>
		</tr>
		<tr>
 ...
 %> </label></td>
		...
		<tr>
			...
			<td><textarea class="ta2" name="pollTask.des"><s:property value="pollTask.des"/></textarea></td>
		</tr>
		<tr>
			...
			<td><a
				...
				style="margin:20px 0 0 0;">返回</a>
			</td>
		</tr>
	</table>
	<input type="hidden" id="hid" name="message" value="<s:property value="str"/>"/>
	</form>
	<div>
